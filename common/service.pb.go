...
const (
	...
	BackendType_SINGBOX   BackendType = 3
	// 4 was L2TP before the ikev2/l2tp drop and stayed unused since, so the
	// restored backend takes its old number back.
...
type Openvpn struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Decimal string of the client's certificate serial number. OpenVPN exposes
	// the peer certificate serial as decimal (tls_serial_0), so this has to
	// match that representation exactly for the node to identify the user.
...
type Ikev2 struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// CHAP credentials for the L2TP/PPP layer. The message keeps its ikev2
	// name (and Proxy field 8) from before the drop, because that is the key
	// the panel's stored proxy settings still use.
...
type User struct {
	...
	// How many connections this user may hold at once, across every backend and
	// every node. 0 means no limit, which is also what an older panel that does
	// not set the field sends.
	...
	// direction. 0 means no limit. Enforced with tc, and only for the backends
	// that give each client its own tunnel address (openvpn, wireguard,
	// l2tp/ikev2) — xray and sing-box users share the proxy's own egress, so
	// they cannot be told apart at the packet level.
...
const file_common_service_proto_rawDesc = "" +
	...
	"\x04last\x18\x03 \x01(\bR\x04last*J\n" +
	"\vBackendType\x12\b\n" +
	"\x04XRAY\x10\x00\x12\r\n" +
	"\tWIREGUARD\x10\x01\x12\v\n" +
	"\aOPENVPN\x10\x02\x12\v\n" +
	"\aSINGBOX\x10\x03\x12\b\n" +
	"\x04L2TP\x10\x04*_\n" +
	"\bStatType\x12\r\n" +
...
var file_common_service_proto_depIdxs = []int32{
	...
	22, // 12: service.Proxy.openvpn:type_name -> service.Openvpn
	23, // 13: service.Proxy.ikev2:type_name -> service.Ikev2
