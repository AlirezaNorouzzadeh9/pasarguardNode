#!/usr/bin/env bash
#
# PasarGuard Node (multi-backend) container entrypoint.
...
DATA="${PG_NODE_DATA:-/var/lib/pg-node}"
...
if [ ! -s "$SSL_CERT_FILE" ]; then
  ...
  echo "[pg-node] generated TLS certificate for ${PUBLIC_IP}"
fi
...
echo "================= PasarGuard Node — Server CA ================="
