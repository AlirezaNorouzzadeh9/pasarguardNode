...
// The image moved from Debian to Alpine while this backend was parked on the
// keep branch, and the two distros install charon in different places. Resolve
// once at startup instead of hard-coding either.
var (
	charonBinary  = firstExisting("/usr/lib/ipsec/charon", "/usr/lib/strongswan/charon", "/usr/libexec/ipsec/charon")
	swanctlBinary = firstExisting("/usr/sbin/swanctl", "/usr/bin/swanctl")
)

// firstExisting returns the first path that exists, or the first candidate so
// the CheckDeps error still names a sensible path when none are installed.
